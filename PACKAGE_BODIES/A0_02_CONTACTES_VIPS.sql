--------------------------------------------------------
--  DDL for Package Body A0_02_CONTACTES_VIPS
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "SINTAGMA_U"."A0_02_CONTACTES_VIPS" AS
    ...
    BEGIN              
        ...
        SELECT A0_SEQ_CONTACTE.NEXTVAL AS ID_CONTACTE, 
                   N_VIP, 
          ...
          */
          /* ANULADO -- AHOIRA ES : 1 CONTACTE = N_VIP + CARREC+ ENTITAT + ADRECA + MUNICIPI
...
--                  AND CV.CLASSIF = act.codi
   ...
   PROCEDURE SB30_TRACTAMENTS IS 
            ...
            BEGIN
 /* SE ARREGLA. ESTABA COGIENDO HR.S COMO DUPLICADO            
...
*/
           INSERT INTO Z_SB030_TRACTAMENT (ID, CODI, DESCRIPCIO, DATA_CREACIO, DATA_MODIFICACIO, DATA_ESBORRAT, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO,ABREUJADA,ID_ORIGINAL,ESQUEMA_ORIGINAL,TABLA_ORIGINAL) 
                     SELECT A0_SEQ_DM_TRACTAMENT.NEXTVAL AS ID ,  
                            TRACTAMENTS_UNICOS.CODI, 
                            NVL(TRACTAMENTS_UNICOS.descripcio,CODI) AS DESCRIPCIO,
                            FUNC_FECHA_1970(NULL) AS DATA_CREACIO, 
                            FUNC_NULOS_INSERT() AS DATA_MODIFICACIO, 
                            FUNC_NULOS_INSERT() AS DATA_ESBORRAT, 
                            FUNC_USU_CREACIO(NULL) AS USUARI_CREACIO, 
                    ...
                    FROM (
                            SELECT CODI,
                            	   DESCRIPCIO, 
                                   FUNC_NORMALITZAR_TRACTAMENT(DESCRIPCIO) AS normalitzar,
                                   ABREUJADA,
                                   ROW_NUMBER ()  OVER (PARTITION BY FUNC_NORMALITZAR_TRACTAMENT(DESCRIPCIO) ORDER BY FUNC_NORMALITZAR_TRACTAMENT(DESCRIPCIO) ASC ) AS COLNUM
                            FROM Z_TMP_VIPS_U_TRACTAMENTS                           
                         ) TRACTAMENTS_UNICOS
                          WHERE colnum=1
                         AND NOT EXISTS (SELECT 1
                                           FROM Z_SB030_TRACTAMENT NUEVOS
                                          WHERE FUNC_NORMALITZAR_TRACTAMENT(TRACTAMENTS_UNICOS.DESCRIPCIO) = FUNC_NORMALITZAR_TRACTAMENT(NUEVOS.DESCRIPCIO)
                                     );   
            
--         max_seq := max_seq +1;
--         SET_SEQ('A0_SEQ_DM_TRACTAMENT',max_seq);
/*            
            INSERT INTO Z_SB030_TRACTAMENT (ID, CODI, DESCRIPCIO, DATA_CREACIO, DATA_MODIFICACIO, DATA_ESBORRAT, USUARI_CREACIO,USUARI_ESBORRAT, USUARI_MODIFICACIO, ABREUJADA,ID_ORIGINAL,ESQUEMA_ORIGINAL,TABLA_ORIGINAL) 
                         SELECT A0_SEQ_DM_TRACTAMENT.NEXTVAL AS ID , 
...
--                                          WHERE ANTIGUOS.DESCRIPCIO = NUEVOS.DESCRIPCIO);
    ...
    COMMIT;
                INSERT INTO A0_DM_TRACTAMENT (ID, DESCRIPCIO, DATA_CREACIO, DATA_MODIFICACIO, DATA_ESBORRAT, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, ABREUJADA,ID_ORIGINAL,ESQUEMA_ORIGINAL,TABLA_ORIGINAL) 
                     SELECT ID, 
                     		DESCRIPCIO, 
                     		DATA_CREACIO, 
                     		DATA_MODIFICACIO, 
    ...
    BEGIN
            ...
            INSERT INTO Z_SB050_CARRECS (ID_CARREC,CARREC_NORM,ID_ORIGINAL,ESQUEMA_ORIGINAL,TABLA_ORIGINAL)
                   SELECT A0_SEQ_DM_CARREC.NEXTVAL AS ID_CARREC, 
...
--                                  (SELECT tractament.ID FROM A0_DM_TRACTAMENT tractament WHERE tractament.ID_ORIGINAL = Subjecte.Tractament) AS TRACTAMENT_ID,
                                  (CASE WHEN (SELECT Tractament.ID FROM A0_DM_TRACTAMENT Tractament WHERE Tractament.ID_ORIGINAL = Subjecte.TRACTAMENT ) IS NULL THEN
                                            (SELECT ID
                                            FROM A0_DM_TRACTAMENT
                                            WHERE FUNC_NORMALITZAR_TRACTAMENT(DESCRIPCIO) = (
                                                                                              SELECT FUNC_NORMALITZAR_TRACTAMENT(DESCRIPCIO)
                                                                                                FROM Z_TMP_VIPS_U_TRACTAMENTS
                                                                                                WHERE CODI =(
                                                                                                                        SELECT TRACTAMENTS_VIPS.TRACTAMENT AS CODI 
                                                                                                                        FROM (	
                                                                                                                                SELECT TRACTAMENT AS TRACTAMENT
                                                                                                                                  FROM Z_TMP_VIPS_U_VIPS
                                                                                                                                 WHERE FUNC_NORMALITZAR(TRACTAMENT) NOT IN ( SELECT FUNC_NORMALITZAR(CODI)
                                  ...
                                  ELSE                                                                          
                                             (SELECT Tractament.ID FROM A0_DM_TRACTAMENT Tractament WHERE Tractament.ID_ORIGINAL = Subjecte.TRACTAMENT)
                                  END)AS TRACTAMENT_ID, 
    ...
    INSERT INTO Z_SB073_SUBJECTE_CONTACTE (ID, NOM,COGNOM1,COGNOM2,ALIES,NOM_NORMALITZAT,DATA_DEFUNCIO,MOTIU_BAIXA,DATA_CREACIO,DATA_ESBORRAT,DATA_MODIFICACIO,USUARI_CREACIO, USUARI_MODIFICACIO, USUARI_ESBORRAT, TRACTAMENT_ID,	PRIORITAT_ID, TIPUS_SUBJECTE_ID, AMBIT_ID, IDIOMA_ID, ID_ORIGINAL, ESQUEMA_ORIGINAL, TABLA_ORIGINAL)
                ...
                FROM (
                          SELECT  Contactes.N_VIP AS ID,
                                  ...
                                  (CASE WHEN (SELECT Tractament.ID FROM A0_DM_TRACTAMENT Tractament WHERE Tractament.ID_ORIGINAL = VIPS.TRACTAMENT ) IS NULL THEN
                                            (SELECT ID
                                            FROM A0_DM_TRACTAMENT
                                            WHERE FUNC_NORMALITZAR_TRACTAMENT(DESCRIPCIO) = (
                                                                                              SELECT FUNC_NORMALITZAR_TRACTAMENT(DESCRIPCIO)
                                  ...
                                  ELSE                                                                          
                                             (SELECT Tractament.ID FROM A0_DM_TRACTAMENT Tractament WHERE Tractament.ID_ORIGINAL = VIPS.TRACTAMENT)
                                  END)AS TRACTAMENT_ID, 
--                                  (SELECT Prioritat.ID FROM DM_PRIORITAT Prioritat WHERE Prioritat.DESCRIPCIO = SubjectePR.prioritat) AS PRIORITAT_ID,
              ...
              WHERE NOT EXISTS (SELECT 1 
                ...
                AND ID NOT IN (SELECT ID_ORIGINAL FROM Z_SB072_SUBJECTES_VIPS)  -- PARA EVITAR REPETICIONES DE ID DESDE CLASSIF (CPONTACTES) ID DE VIPS (SUBJECTO) QUE YA SE HAN MIGRADO
              GROUP BY ID;
    ...
    PROCEDURE SB79_ERR_SUBJECTE IS
    BEGIN
      ...
      BEGIN
              INSERT INTO Z_SB080_ADRECA_CONTACTE (ID_ADRECA,ID_CONTACTE,ADRECA,MUNICIPI,CP,PROVINCIA,PAIS,USU_ALTA,DATA_ALTA,USU_MODI,DATA_MODI,ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL)
                    SELECT A0_SEQ_ADRECA.NEXTVAL AS ID_ADRECA, 
                           ...
                           N_VIP AS ID_ORIGINAL, 
                           'VIPS_U' AS ESQUEMA_ORIGINAL , 
                           'CLASSIF_VIP' AS TABLA_ORIGINAL
                   FROM (
                           SELECT Contactes.ID_CONTACTE AS ID_CONTACTE, 
                               MAX(Contactes.N_VIP) AS N_VIP,
    ...
    BEGIN
     ...
     COMMIT;
              INSERT INTO A0_ADRECA (ID, NOM_CARRER, MUNICIPI, PROVINCIA, CODI_POSTAL, PAIS, USUARI_CREACIO, DATA_CREACIO, USUARI_MODIFICACIO,DATA_MODIFICACIO,ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL)
                 SELECT ID, 
                        NOM_CARRER, 
                        MUNICIPI, 
                        PROVINCIA, 
              ...
              WHERE NOT EXISTS (SELECT 1 
               ...
               -- OCG1 // APS1
               AND FUNC_SIGNES_LITERAL(NOM_CARRER) IS NOT NULL;
              
              COMMIT;
                     
    END;
        
     PROCEDURE SB83_ERR_ADRECA  IS
     BEGIN
        ...
        INSERT INTO ERR_ADRECA (ID, NOM_CARRER, ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL,ERROR_DESCRIPCION)
                 SELECT ID, 
                   ...
                   FROM Z_SB999_ADRECA ADRECA
               -- OCG1 // APS1
               WHERE FUNC_SIGNES_LITERAL(NOM_CARRER) IS NULL;
...
--                AND TO_NUMBER(SUBSTR(PAIS,1,1)) IN (0,1,2,3) AND TRIM(PAIS)<>'.'
        ...
        INSERT INTO Z_SB094_CONTACTES_SUBJECTES (ID, ES_PRINCIPAL, CARREC_ID, DEPARTAMENT, DADES_QUALITAT, DATA_DARRERA_ACTUALITZACIO, DATA_CREACIO, DATA_MODIFICACIO, DATA_ESBORRAT, USUARI_CREACIO, USUARI_MODIFICACIO, USUARI_ESBORRAT, TIPUS_CONTACTE_ID, SUBJECTE_ID, ADRECA_ID, ENTITAT_ID,VISIBILITAT_ID, AMBIT_ID,ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL)
          ...
          FROM (
                    SELECT B004.ID_CONTACTE, 
                           ...
                           ConstVISIBILITAT_ALCALDIA AS VISIBILITAT_ID  ,
                           ConstAMBIT_PROTOCOL AS AMBIT_ID,
                           B004.N_VIP AS N_VIP_ID  --ESTE ES EL N_VIP
                    FROM Z_SB004_SUBJECTES_ID B004, 
                         ...
                         Z_SB066_ENTITATS_CONTACTES Entitats, 
                         A0_SUBJECTE Subjecte
                    WHERE B004.n_vip = Subjecte.ID_original
    ...
    BEGIN
       ...
       INSERT INTO A0_CONTACTE (ID, ES_PRINCIPAL, CARREC_ID, DEPARTAMENT, DADES_QUALITAT, DATA_DARRERA_ACTUALITZACIO, DATA_CREACIO, DATA_MODIFICACIO, DATA_ESBORRAT, USUARI_CREACIO, USUARI_MODIFICACIO, USUARI_ESBORRAT, TIPUS_CONTACTE_ID, SUBJECTE_ID, ADRECA_ID, ENTITAT_ID,VISIBILITAT_ID, AMBIT_ID,ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL) 
          ...
          FROM (       
            ...
            -- OCG1 // APS1                 SE INSERTAN SÓLO LOS CONTACTES QUE TIENEN SUBJECTE Y DIRECCIÓN  QUE SE HAYAN INSERTADO                         
             AND EXISTS (SELECT 1 
            ...
            -- OCG1 // APS1                 SE INSERTAN SÓLO LOS CONTACTES QUE TIENEN SUBJECTE Y DIRECCIÓN  QUE SE HAYAN INSERTADO
             ...
             -- OCG1 // APS1                 ESTOS SON LOS ConstContacteProfesional, QUE VIENEN DE CLASSIF_VIP
    ...
    PROCEDURE SB96_ERR_CONTACTES IS
    BEGIN
        ...
        INSERT INTO ERR_CONTACTE (ID, NOM,COGNOMS, TIPUS_CONTACTE_ID, CARREC_ID, ENTITAT_ID, ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL, ERR_DESCRIPCION)
              ...
              FROM (
                         SELECT ID AS ID,
                         	    VIP.NOM,
                         	    VIP.COGNOMS,TIPUS_CONTACTE_ID,
                                CARREC_ID,
                                ENTITAT_ID,          
                                ID_ORIGINAL AS ID_ORIGINAL, 
                                ESQUEMA_ORIGINAL AS ESQUEMA_ORIGINAL, 
                                TABLA_ORIGINAL AS TABLA_ORIGINAL 
                        FROM Z_SB093_CONTACTES_CONTACTES CONTACTES_PROF,
                             Z_TMP_VIPS_U_VIPS VIP
                        WHERE VIP.N_VIP = CONTACTES_PROF.ID_ORIGINAL     
                     ) NUEVOS   
            ...
            INSERT INTO ERR_CONTACTE (ID, NOM,COGNOMS, TIPUS_CONTACTE_ID, CARREC_ID, ENTITAT_ID, ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL, ERR_DESCRIPCION)
              ...
              FROM (
                         SELECT ID AS ID,
                                ...
                                CARREC_ID,
                                ENTITAT_ID,     
                                CONTACTES_PROF.SUBJECTE_ID AS SUBJECTE_ID,
                                ID_ORIGINAL AS ID_ORIGINAL, 
                                ESQUEMA_ORIGINAL AS ESQUEMA_ORIGINAL, 
                                TABLA_ORIGINAL AS TABLA_ORIGINAL 
                        FROM Z_SB093_CONTACTES_CONTACTES CONTACTES_PROF,
           ...
           COMMIT; 
           /*SB0'94 CONTACTE QUE NO SE INSERTA PORQUE SU SUBJECTES QUE NO EXISTE */
           INSERT INTO ERR_CONTACTE (ID, NOM,COGNOMS, TIPUS_CONTACTE_ID, CARREC_ID, ENTITAT_ID, ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL, ERR_DESCRIPCION)
              SELECT ID AS ID,   
              	     NOM,
                     COGNOMS,              
              ...
              FROM (
                         SELECT ID AS ID,
                         	    VIP.NOM,
                         	    VIP.COGNOMS,TIPUS_CONTACTE_ID,
                                CONTACTES_PERSONA.SUBJECTE_ID AS SUBJECTE_ID,
                                CARREC_ID,
                                ENTITAT_ID,          
                                ID_ORIGINAL AS ID_ORIGINAL, 
                                ESQUEMA_ORIGINAL AS ESQUEMA_ORIGINAL, 
                                TABLA_ORIGINAL AS TABLA_ORIGINAL 
                        FROM Z_SB094_CONTACTES_SUBJECTES CONTACTES_PERSONA,
                             Z_TMP_VIPS_U_VIPS VIP
                        WHERE VIP.N_VIP = CONTACTES_PERSONA.ID_ORIGINAL     
                     ) NUEVOS   
           ...
           /* SB093 ADRECA  INSERTAMOS LOS CONTACTES QUE NO SE METEN PORQUE LA DIRECCIÓN DEL SUBJECTE NO SE HA INTRODUCIDO*/
            INSERT INTO ERR_CONTACTE (ID, NOM,COGNOMS, TIPUS_CONTACTE_ID, CARREC_ID, ENTITAT_ID, ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL, ERR_DESCRIPCION)
              SELECT ID AS ID,   
              	     NOM,
                     COGNOMS,              
              ...
              FROM (
                         SELECT ID AS ID,
                                ...
                                CARREC_ID,
                                ENTITAT_ID,
                                CONTACTES_PROF.ADRECA_ID AS ADRECA_ID,
                                ID_ORIGINAL AS ID_ORIGINAL, 
                                ESQUEMA_ORIGINAL AS ESQUEMA_ORIGINAL, 
                                TABLA_ORIGINAL AS TABLA_ORIGINAL 
                        FROM Z_SB093_CONTACTES_CONTACTES CONTACTES_PROF,
           ...
           COMMIT;
            ...
            /*SB0'94 ADRECA  CONTACTE QUE NO SE INSERTA PORQUE SU ADRECA NO EXISTE */
           INSERT INTO ERR_CONTACTE (ID, NOM,COGNOMS, TIPUS_CONTACTE_ID, CARREC_ID, ENTITAT_ID, ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL, ERR_DESCRIPCION)
              SELECT ID AS ID,   
              	     NOM,
                     COGNOMS,              
              ...
              FROM (
                         SELECT ID AS ID,
                                ...
                                CARREC_ID,
                                ENTITAT_ID,
                                CONTACTES_PERSONA.ADRECA_ID AS ADRECA_ID,
                                ID_ORIGINAL AS ID_ORIGINAL, 
                                ESQUEMA_ORIGINAL AS ESQUEMA_ORIGINAL, 
                                TABLA_ORIGINAL AS TABLA_ORIGINAL 
                        FROM Z_SB094_CONTACTES_SUBJECTES CONTACTES_PERSONA,
    ...
    BEGIN
        ...
        BEGIN
            ...
            INSERT INTO A0_CONTACTE_CORREU(ID, CORREU_ELECTRONIC, ES_PRINCIPAL, DATA_CREACIO, USUARI_CREACIO, Contacte_Id,ID_ORIGINAL, ESQUEMA_ORIGINAL, TABLA_ORIGINAL)
                    SELECT A0_SEQ_CONTACTE_CORREU.NEXTVAL AS ID, 
                     ...
                     WHERE NOT EXISTS(SELECT 1 
                                      FROM A0_CONTACTE_CORREU correu
                                      WHERE correu.Contacte_Id = cc.id_contacte
                                        AND CORREU_ELECTRONIC = CC.CORREU)
                      and exists (select 1 from A0_contacte contacte
                                    where  contacte.Id = cc.id_contacte
                                 );   
   ...
   BEGIN
           ...
           INSERT INTO Z_SB111_TELEFON_PARTICULAR (ID_CONTACTE, N_VIP, TIPUS_TELEFON, TELEFON, ES_PRINCIPAL, CONCEPTE, ES_PARTICULAR, ID_ORIGINAL, ESQUEMA_ORIGINAL, TABLA_ORIGINAL)
                        ...
                        FROM (	   
                         ...
                         WHERE NOT EXISTS (SELECT 1
                                             FROM Z_SB111_TELEFON_PARTICULAR ANTIGUOS
                                            WHERE ANTIGUOS.ID_CONTACTE = NUEVOS.ID_CONTACTE
                                              AND ANTIGUOS.N_VIP = NUEVOS.N_VIP
                                              AND ANTIGUOS.TIPUS_TELEFON = NUEVOS.TIPUS_TELEFON
                                              AND ANTIGUOS.TELEFON = NUEVOS.TELEFON
    ...
    COMMIT;
     ...
     INSERT INTO A0_CONTACTE_TELEFON (ID, NUMERO, ES_PRINCIPAL, OBSERVACIONS, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, CONTACTE_ID, TIPUS_TELEFON_ID, CONCEPTE, ES_PRIVAT, ID_ORIGINAL, ESQUEMA_ORIGINAL, TABLA_ORIGINAL)
                    SELECT A0_SEQ_CONTACTE_TELEFON.NEXTVAL AS ID, 
                           ...
                           NUEVOS.ESQUEMA_ORIGINAL AS ESQUEMA_ORIGINAL,
                           NUEVOS.TABLA_ORIGINAL AS TABLA_ORIGINAL
                     FROM Z_SB111_TELEFON_PARTICULAR NUEVOS 
                     WHERE NUEVOS.TELEFON IS NOT NULL                       
                       AND NOT EXISTS(SELECT 1 
                                      FROM A0_CONTACTE_TELEFON ANTIGUOS
                                      WHERE ANTIGUOS.numero = NUEVOS.telefon
...
*/    
    ...
    BEGIN
          ...
          INSERT INTO Z_SB120_CLASSIFICACIONS (ID, CODI, DESCRIPCIO, ORDEN, ID_ORIGINAL, ESQUEMA_ORIGINAL, TABLA_ORIGINAL)
                SELECT A0_SEQ_DM_CLASSIFICACIO.NEXTVAL AS ID,
                       CLASSIF.CODI, 
                       CLASSIF.DESCRIPCIO, 
                       LISTA.orden,
    ...
    BEGIN
          ...
          INSERT INTO A0_DM_CLASSIFICACIO (ID, DESCRIPCIO, CODI, PRELACIO, DATA_INICI_REVISO, DATA_FI_REVISIO, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, TIPOLOGIA_ID, AMBIT_ID, ID_ORIGINAL, ESQUEMA_ORIGINAL, TABLA_ORIGINAL)
                SELECT B120.ID AS ID,
                       ...
                       FUNC_FECHA_CREACIO(NULL) AS DATA_CREACIO, 
--                       (case when (select  CODI from TMP_CLASSIF_ACTIVES) is null NULL AS DATA_ESBORRAT,
                        (case when (select  ACTIVES.CODI from TMP_CLASSIF_ACTIVES ACTIVES WHERE ACTIVES.CODI = B120.CODI) is null then 
    ...
    BEGIN
      ...
      INSERT INTO Z_SB130_CLASSIF_CONTACTE (CONTACTE_ID, CLASSIFICACIO_CODI, PRELACIO, ID_CLASSIFICACIO, DATA_CREACIO, USUARI_CREACIO, USUARI_MODIFICACIO, DATA_MODIFICACIO,ID_ORIGINAL, ESQUEMA_ORIGINAL, TABLA_ORIGINAL)
                SELECT ID_CONTACTE AS CONTACTE_ID, 
                     ...
                     'CLASSIF_VIPS' AS TABLA_ORIGINAL                     
              FROM Z_SB003_CONTACTES_ID CONTACTES, 
                   A0_DM_CLASSIFICACIO CLASSIF
              WHERE CLASSIF.CODI = CONTACTES.CLASSIF             
                AND EXISTS (SELECT 1 
                            FROM A0_CONTACTE CONTACTE
    ...
    BEGIN
    /*
        INSERT INTO A0_CONTACTE_CLASSIFICACIO (ID, PRELACIO, DATA_FI_VIGENCIA, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, CLASSIFICACIO_ID, CONTACTE_ID, ID_ORIGINAL, ESQUEMA_ORIGINAL, TABLA_ORIGINAL)
              SELECT A0_SEQ_CONTACTE_CLASSIFICACIO.NEXTVAL AS ID, 
    ...
    */
               ...
               INSERT INTO A0_CONTACTE_CLASSIFICACIO (ID, PRELACIO, DATA_FI_VIGENCIA, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, CLASSIFICACIO_ID, CONTACTE_ID, ID_ORIGINAL, ESQUEMA_ORIGINAL, TABLA_ORIGINAL)
                     SELECT A0_SEQ_CONTACTE_CLASSIFICACIO.NEXTVAL AS ID, 
                             ...
                             TABLA_ORIGINAL AS TABLA_ORIGINAL
                      FROM Z_SB130_CLASSIF_CONTACTE NUEVOS,
                           TMP_CLASSIF_ACTIVES activas                           
                      WHERE activas.codi = nuevos.classificacio_codi
                        AND NOT EXISTS(SELECT 1 
                             FROM A0_CONTACTE_CLASSIFICACIO ANTIGUOS 
                            WHERE ANTIGUOS.CONTACTE_ID = NUEVOS.CONTACTE_ID
                             AND  ANTIGUOS.classificacio_ID = NUEVOS.id_classificacio
        ...
        INSERT INTO Z_SB140_ACOMPANYANTS_VIPS (SUBJECTE_ID, NOM, COGNOM1, COGNOM2, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, TRACTAMENT_ID, CONTACTE_ID, ID_ORIGINAL, ESQUEMA_ORIGINAL, TABLA_ORIGINAL)
                       ...
                       SELECT SUBJECTE.ID AS SUBJECTE_ID,
                             ...
                             (CASE WHEN trim(SUBSTR(VIPS.COGNOMS_A,1, INSTR(LOWER(VIPS.COGNOMS_A),' i ')-1)) IS NOT NULL THEN 
                              ...
                              (SELECT Tractament.ID FROM A0_DM_TRACTAMENT Tractament WHERE Tractament.ID_ORIGINAL = VIPS.TRACTAMENT_A ) AS TRACTAMENT_ID, 
    ...
    BEGIN
          ...
          INSERT INTO A0_ACOMPANYANT (ID, NOM, COGNOM1, COGNOM2, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, TRACTAMENT_ID, CONTACTE_ID, ID_ORIGINAL, ESQUEMA_ORIGINAL, TABLA_ORIGINAL)
              SELECT A0_SEQ_ACOMPANYANT.NEXTVAL AS ID, 
                     NOM, 
    ...
    BEGIN    
       ...
       DELETE FROM Z_SB111_TELEFON_PARTicular;
       ...
       DELETE FROM ERR_SUBJECTE;
       DELETE FROM ERR_ADRECA;
       DELETE FROM ERR_CONTACTE;
       DELETE FROM ERR_TRUCADA;
    ...
    BEGIN
        ...
        --DROP SEQUENCE A0_SEQ_xxx;
        --CREATE SEQUENCE A0_SEQ_xxx MINVALUE 10000 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 10000 NOCACHE NOORDER NOCYCLE;
        FOR i in 1 .. total LOOP
            -- NOMBRE : A0_SEQ_DM_XXX        Nímero al cuál resetear
