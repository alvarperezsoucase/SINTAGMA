--------------------------------------------------------
--  DDL for Package Body A0_04_CONTACTES_RISPLUS
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "SINTAGMA_U"."A0_04_CONTACTES_RISPLUS" AS
            ...
            SELECT CONTACTE_ID, ENTITAT_ID, NUEVOS.DESCRIPCIO_ENTITAT, NUEVOS.NOM_NORMALIZAT 
               FROM (
                     ...
                     FROM (
                            ...
                            FROM Z_T_RP_CONTACTES
                            WHERE empresa IS NOT null  
                            GROUP BY FUNC_NORMALITZAR_ENTITAT(EMPRESA)
                            ORDER BY 1
                          ) ENTITAT,
                            A0_DM_ENTITAT DM_ENTITAT,
                            Z_T_RP_CONTACTES CONTACTE
    ...
    BEGIN
        ...
        INSERT INTO  Z_SD011_A_ENTITAT_NO_EXISTENTE (ENTITAT_ID, NOM_NORMALIZAT)
             ...
             FROM (
                    ...
                    FROM Z_T_RP_CONTACTES CONTACTES
                    WHERE EMPRESA IS NOT NULL  
...
*/   
   ...
   PROCEDURE SD28_ERR_SUBJECTE IS
    BEGIN
        ...
        INSERT INTO ERR_SUBJECTE (ID, NOM, COGNOM1, COGNOM2, TRACTAMENT_ID, ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL,ERROR_DESCRIPCION)
                ...
                FROM Z_SD999_SUBJECTES NUEVOS
                -- OCG1 // APS1
                WHERE FUNC_SIGNES_LITERAL(NOM) IS NULL
    ...
    BEGIN
             INSERT INTO Z_SD999_ADRECA (ID, CODI_MUNICIPI, MUNICIPI, CODI_PROVINCIA, PROVINCIA, CODI_PAIS, PAIS, CODI_CARRER, NOM_CARRER, LLETRA_INICI, LLETRA_FI, ESCALA, CODI_POSTAL, COORDENADA_X, COORDENADA_Y, SECCIO_CENSAL, ANY_CONST, NUMERO_INICI, NUMERO_FI, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, PIS, PORTA, BLOC, CODI_TIPUS_VIA, CODI_BARRI, BARRI, CODI_DISTRICTE, DISTRICTE,ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL)
					SELECT  ID_ADRECA AS ID, 
                            ...
                            (SELECT CONTACTEID  FROM  Z_SD041_ADRECAS_CONTACTES WHERE ID_ADRECA = NUEVAS.ID_ADRECA AND ROWNUM=1) AS  ID_ORIGINAL, 
                            'RISPLUS' AS ESQUEMA_ORIGINAL , 
                            'CONTACTES' AS TABLA_ORIGINAL
             ...
             INSERT INTO A0_ADRECA (ID, CODI_MUNICIPI, MUNICIPI, CODI_PROVINCIA, PROVINCIA, CODI_PAIS, PAIS, CODI_CARRER, NOM_CARRER, LLETRA_INICI, LLETRA_FI, ESCALA, CODI_POSTAL, COORDENADA_X, COORDENADA_Y, SECCIO_CENSAL, ANY_CONST, NUMERO_INICI, NUMERO_FI, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, PIS, PORTA, BLOC, CODI_TIPUS_VIA, CODI_BARRI, BARRI, CODI_DISTRICTE, DISTRICTE,ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL)
                SELECT ID,
                       ...
                       ID_ORIGINAL AS ID_ORIGINAL, 
                        ...
                        FROM Z_SD999_ADRECA
                          -- OCG1 // APS1
               WHERE FUNC_SIGNES_LITERAL(NOM_CARRER) IS NOT NULL;
    
    COMMIT;
    END;
    
    
      PROCEDURE SD43_ERR_ADRECA  IS
     BEGIN
     ...
     /*
        INSERT INTO ERR_ADRECA (ID, NOM_CARRER, ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL,ERROR_DESCRIPCION)
                 SELECT ID, 
                   ...
                   FROM Z_SD999_ADRECA ADRECA              
               -- OCG1 // APS1
               AND FUNC_SIGNES_LITERAL(NOM_CARRER) IS NULL;
        */               
           
           INSERT INTO ERR_ADRECA (ID, NOM_CARRER, ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL,ERROR_DESCRIPCION)    
                       SELECT CONTACTES.CONTACTEID AS ID,
                       CONTACTES.ADRECA AS NOM_CARRER,
                       CONTACTEID AS ID_ORIGINAL,
                       'RISPLUS' AS ESQUEMA_ORIGINAL,
						'CONTACTES' AS TABLA_ORIGINAL,						
						'ADRECA NULL'  AS ERROR_DESCRIPCION
                FROM Z_T_RP_CONTACTES CONTACTES 
               WHERE  FUNC_SIGNES_LITERAL(CONTACTES.ADRECA) IS NULL ;
    ...
    BEGIN
            ...
            SELECT A0_SEQ_CONTACTE.NEXTVAL AS ID,
                    ...
                    (CASE WHEN (EMPRESA IS NOT NULL) THEN
                        ConstContacteProfesional
                    ELSE 
                        ConstContactePersonal
...
--                   (SELECT D011.ENTITAT_ID FROM Z_SD011_B_ENTITAT_NO_EXIST D011 WHERE D011.CONTACTEID = CONTACTEID and ROWNUM = 1) AS ENTITAT_ID,
                   (SELECT ENTITAT.ID FROM A0_DM_ENTITAT ENTITAT WHERE FUNC_NORMALITZAR(ENTITAT.DESCRIPCIO) = FUNC_NORMALITZAR(EMPRESA) and ROWNUM = 1) AS ENTITAT_ID,
                    FUNC_NULOS_STRING() AS CONTACTE_ORIGEN_ID,
                    ConstVisibilitatPrivada AS VISIBILITAT_ID,
                    ConstAlcaldia AS AMBIT_ID,
                    (SELECT id FROM A0_DM_CARREC WHERE FUNC_NORMALITZAR(CARREC) = FUNC_NORMALITZAR(DESCRIPCIO) and ROWNUM = 1) AS CARREC_ID,
                    CONTACTEID AS ID_ORIGINAL,
                    'RISPLUS' AS ESQUEMA_ORIGINAL,
                    'CONTACTES' AS TABLA_ORIGINAL,
    ...
    END;
      ...
      PROCEDURE SD55_ERR_CONTACTES IS
    BEGIN
        ...
        INSERT INTO ERR_CONTACTE (ID, NOM,COGNOMS, TIPUS_CONTACTE_ID, CARREC_ID, ENTITAT_ID, ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL, ERR_DESCRIPCION)
              ...
              FROM (
                        ...
                        FROM Z_SD999_CONTACTES CONTACTES,
                             Z_SD999_SUBJECTES SUBJECTE
...
/*    
    ...
    PROCEDURE SD71_TELEFON_PRINCIPAL IS
      ...
      BEGIN
           ...
           INSERT INTO A0_CONTACTE_TELEFON (ID, NUMERO, ES_PRINCIPAL, DATA_CREACIO, USUARI_CREACIO, CONTACTE_ID, TIPUS_TELEFON_ID,ID_ORIGINAL, ESQUEMA_ORIGINAL , TABLA_ORIGINAL)
        ...
        INSERT INTO Z_SD025_SUBJECTES (SINTAGMA_SUBJECTE_ID, CONTACTE_RISPLUS_ID, TRACTAMENT, TRACTAMENT_ID, NOM, COGNOM1, COGNOM2, NOM_NORMALITZAT, MOTIU_BAIXA, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, PRIORITAT_ID, TIPUS_SUBJECTE_ID, AMBIT_ID, IDIOMA)        
                ...
                WHERE CARREC is NULL and EMPRESA IS NULL 
                  AND NOT EXISTS (SELECT 1 
                                  ...
                                  WHERE (ConstSubjecte + Z_T_RP_CONTACTES.CONTACTEID) = ANTIGUOS.ID_SUBJECTE
                                 ); 
 -------------------------------------------------------------------------               
    ...
    */
    ...
    /*
        DELETE FROM Z_SD071_TELEFONS_NUMERICS;
        DELETE FROM Z_SD070_TELEFONS_NUMERICS;
   */ 
