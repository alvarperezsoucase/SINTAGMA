--------------------------------------------------------
--  DDL for Package Body MIGRACIO_RISPLUS
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "SINTAGMA_U"."MIGRACIO_RISPLUS" AS
...
*/   
   ...
   BEGIN
        ...
        INSERT INTO Z_D99_SUBJECTES (ID, NOM, COGNOM1, COGNOM2, ALIES, DATA_DEFUNCIO, ES_PROVISIONAL, NOM_NORMALITZAT, MOTIU_BAIXA, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, TRACTAMENT_ID, PRIORITAT_ID, TIPUS_SUBJECTE_ID, AMBIT_ID, IDIOMA_ID)
                     SELECT NUEVOS.ID AS ID,
                            NUEVOS.NOM AS NOM,
                            NUEVOS.COGNOM1 AS COGNOM1,
                            NUEVOS.COGNOM2 AS COGNOM2,
                            NULL AS ALIES,
                            NULL AS DATA_DEFUNCIO,
                            0 AS ES_PROVISIONAL,
                            NUEVOS.NOM_NORMALITZAT,
                            NULL AS MOTIU_BAIXA,
                            SYSDATE AS DATA_CREACIO,
                            NULL AS DATA_ESBORRAT,
                            ...
                            (SELECT ID FROM AUX_DM_TRACTAMENT WHERE DESCRIPCIO = NUEVOS.TRACTAMENT) AS TRACTAMENT_ID,
                            0 AS PRIORITAT_ID,
                ...
                INSERT INTO AUX_SUBJECTE
                            SELECT * 
                            FROM Z_D99_SUBJECTES NUEVOS
                           WHERE NOT EXISTS(SELECT 1
                                              FROM AUX_SUBJECTE ANTIGUOS
...
*/    
    ...
    BEGIN
            INSERT INTO Z_D99_ADRECA (ID, CODI_MUNICIPI, MUNICIPI, CODI_PROVINCIA, PROVINCIA, CODI_PAIS, PAIS, CODI_CARRER, NOM_CARRER, LLETRA_INICI, LLETRA_FI, ESCALA, CODI_POSTAL, COORDENADA_X, COORDENADA_Y, SECCIO_CENSAL, ANY_CONST, NUMERO_INICI, NUMERO_FI, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, PIS, PORTA, BLOC, CODI_TIPUS_VIA, CODI_BARRI, BARRI, CODI_DISTRICTE, DISTRICTE)
             ...
             INSERT INTO AUX_ADRECA
                SELECT * FROM Z_D99_ADRECA;    
    ...
    BEGIN
        ...
        INSERT INTO AUX_ADRECA 	(ID, CODI_MUNICIPI, MUNICIPI, CODI_PROVINCIA, PROVINCIA, CODI_PAIS, PAIS, CODI_CARRER, NOM_CARRER, LLETRA_INICI, LLETRA_FI, ESCALA, CODI_POSTAL, COORDENADA_X, COORDENADA_Y, SECCIO_CENSAL, ANY_CONST, NUMERO_INICI, NUMERO_FI, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, PIS, PORTA, BLOC, CODI_TIPUS_VIA, CODI_BARRI, BARRI, CODI_DISTRICTE, DISTRICTE)
    ...
    */
	...
	SELECT (ConstContacte + CONTACTEID) AS ID,
			...
			(SELECT ID_ADRECA FROM Z_D41_ADRECAS_CONTACTES WHERE contacteid= contacteid) AS ADRECA_ID,
            (SELECT ENTITAT_ID FROM Z_D15_ENTITAT_CONTACTE WHERE CONTACTE_ID = CONTACTEID) AS ENTITAT_ID,
            NULL AS CONTACTE_ORIGEN_ID,
            ConstVisibilitatPrivada AS VISIBILITAT_ID,
            ConstAlcaldia AS AMBIT_ID,
            (SELECT id FROM DM_CARREC WHERE FUNC_NORMALITZAR(CARREC) = FUNC_NORMALITZAR(DESCRIPCIO)) AS CARREC_ID
       FROM Z_TMP_RISPLUS_U_CONTACTES
       WHERE NOT EXISTS (SELECT 1
       ...
       INSERT INTO AUX_CONTACTE
                  SELECT * 
                    FROM Z_D99_CONTACTES NUEVOS
                  WHERE NOT EXISTS (SELECT 1
                                      FROM AUX_CONTACTE ANTIGUOS
   ...
   END;
    ...
    PROCEDURE D61_CORREUS_PRINCIPALES IS
        ...
        BEGIN
           ...
           INSERT INTO  AUX_CONTACTE_CORREU (ID, CORREU_ELECTRONIC, ES_PRINCIPAL, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, CONTACTE_ID)
...
--                      AND NOT EXISTS (SELECT * FROM Z_C55_TELEFONS_NUMERICS AUX  WHERE C.ID_CONTACTE_SINTAGMA = AUX.ID_CONTACTE_SINTAGMA AND REPLACE(REPLACE(REPLACE(TRIM(C.TEL_FIX),'-',''),' ',''),'.','') = AUX.TELEFON);
    ...
    PROCEDURE D71_TELEFON_PRINCIPAL IS
      ...
      BEGIN
           ...
           INSERT INTO AUX_CONTACTE_TELEFON (ID, NUMERO, ES_PRINCIPAL, DATA_CREACIO, USUARI_CREACIO, CONTACTE_ID, TIPUS_TELEFON_ID)
