--------------------------------------------------------
--  DDL for Package Body Z_LANZAR_AUX_CALIMA
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "SINTAGMA_U"."Z_LANZAR_AUX_CALIMA" AS
...
/*Sujetos que no existen en Calima */
   ...
   BEGIN 
      --DROP SEQUENCE  AUX_SEQ_ENTITAT_CALIMA
      -- CREATE SEQUENCE  AUX_SEQ_ENTITAT_CALIMA MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1000000 NOCACHE NOORDER NOCYCLE
        ...
        INSERT INTO Z_C12_NO_EXISTE_ENTITAT_SUBJ (ID_ENTITAT_SINTAGMA, ID_SUBJECTE, NOM2, COGNOM_1, COGNOM_2, ALIES, NOM_NORMALITZAT, DATA_DEFUNCIO, IDIOMA_ID, DATA_DARRERA, DATA_BAIXA, ID_PRIORITAT, TIPUS_SUBJECTE_ID, AMBIT_ID)
               SELECT (A0_SEQ_DM_ENTITAT.nextval) AS ID_ENTITAT_SINTAGMA, ENTITAT_TODOS.*
    ...
    --OJO A0_DM_CARREC -> CREATE TABLE A0_DM_CARREC AS SELECT * FROM DM_CARREC 
    --CREATE SEQUENCE  AUX_SEQ_CALIMA_DM_CARREC MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1000000 NOCACHE NOORDER NOCYCLE
    ...
    END;
       ...
       --Se buscan los CARREC que NO ESTÁN EN DM_CARREC y se les asigna  un nuevo ID ¿meterlos directamente en DM_CARREC)?
    --OJO A0_DM_CARREC -> CREATE TABLE A0_DM_CARREC AS SELECT * FROM DM_CARREC 
    --DROP SEQUENCE  AUX_SEQ_CALIMA_DM_TRACTAMENT;
    --CREATE SEQUENCE  AUX_SEQ_CALIMA_DM_TRACTAMENT MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1000000 NOCACHE NOORDER NOCYCLE

    PROCEDURE C25_NEW_TRACTAMENT IS 
    BEGIN
            INSERT INTO A0_DM_TRACTAMENT (ID, DESCRIPCIO, ABREUJADA, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO)
    ...
    BEGIN
            INSERT INTO Z_C30_TODOS_CONTACTES (ID_CONTACTE_SINTAGMA, PRC, PART, NP, TRACTAMENT, TRACTAMENT_ID, NOM, NOM2, COGNOM_1, COGNOM_2, ALIES, DNI, IDIOMA, SEXE, DATA_NAIXE, DATA_DEFUNCIO, DATA_BAIXA, BAIXA_REGISTRE, VINC, NC, SINTAGMA_CARREC_ID, CARREC, ORGANISME, DIRECCIO, POBLACIO, PROVINCIA, PAIS, CODI_POSTAL, TEL_FIX, TEL_MOBIL, TEL_FIX_CENTRALETA, TEL_ALTRES, FAX, TEL_VERMELL, DISTRICTE, EMAIL, CODI_ACTIV, ACTIVITAT, SECTOR, ADHOC, IND_NADAL, DATA_EFECTE_CARREC, DATA_DIMISSIO_CARREC, DATADARRERA, DATALTA, MOTIU, OBSERV)
               SELECT CONTACTES_TODOS.NC AS ID_CONTACTE_SINTAGMA,                
                            CONTACTES_TODOS.PRC, 
                            CONTACTES_TODOS.PART, 
                            CONTACTES_TODOS.NP, 
                            CONTACTES_TODOS.TRACTAMENT,
                            (SELECT ID FROM A0_DM_TRACTAMENT WHERE DESCRIPCIO =  CONTACTES_TODOS.TRACTAMENT) AS TRACTAMENT_ID, 
                            CONTACTES_TODOS.NOM, 
                            ...
                            CONTACTES_TODOS.NC, 
                            (SELECT ID FROM A0_DM_CARREC WHERE DESCRIPCIO = CONTACTES_TODOS.CARREC) AS SINTAGMA_CARREC_ID,
    ...
    --DROP SEQUENCE  AUX_SEQ_CALIMA_ADRECA
    --CREATE SEQUENCE  AUX_SEQ_CALIMA_ADRECA MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1000000 NOCACHE NOORDER NOCYCLE
   PROCEDURE C34_AUX_ADRECA_CALIMA IS   
    BEGIN
            INSERT INTO A0_ADRECA (ID, CODI_MUNICIPI, MUNICIPI, CODI_PROVINCIA, PROVINCIA, CODI_PAIS, PAIS, CODI_CARRER, NOM_CARRER, LLETRA_INICI, LLETRA_FI, ESCALA, CODI_POSTAL, COORDENADA_X, COORDENADA_Y, SECCIO_CENSAL, ANY_CONST, NUMERO_INICI, NUMERO_FI, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, PIS, PORTA, BLOC, CODI_TIPUS_VIA, CODI_BARRI, BARRI, CODI_DISTRICTE, DISTRICTE)
					SELECT  (A0_SEQ_ADRECA.NEXTVAL) AS ID, 
   ...
   BEGIN
         INSERT INTO Z_C36_CONTACTE_CON_SUBJECTES 
             SELECT  CONTACTES.ID_CONTACTE_SINTAGMA AS ID,
                ...
                NP AS SUBJECTE_ID,
                (select id from A0_adreca adreca where FUNC_NORMALITZAR(nom_carrer) = FUNC_NORMALITZAR(Direccio)
             ...
             WHERE 
                CONTACTES.NP IN (SELECT ID_Subjecte FROM Z_C04_IDSUBJECTE_SINTAGMA)
               AND NOT EXISTS (SELECT * 
...
--                                WHERE NUEVOS.ID = Z_C03_NO_EXISTE_SUBJECTE.ID_SUBJECTE_SINTAGMA);
   ...
   PROCEDURE C41_AUX_SUBJECTE IS
   BEGIN
   
    INSERT INTO A0_SUBJECTE (ID, NOM, COGNOM1, COGNOM2, ALIES, DATA_DEFUNCIO, ES_PROVISIONAL, NOM_NORMALITZAT, MOTIU_BAIXA, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, TRACTAMENT_ID, PRIORITAT_ID, TIPUS_SUBJECTE_ID, AMBIT_ID, IDIOMA_ID)
          SELECT ID, 
                  NOM, 
                  COGNOM1, 
                  SUBSTR (COGNOM2,1,40),
       ...
       -- MIRAR PORQUE HAY CONTACTOS QUE NO EXISTEN EN ORIGEN y SI EN Z_C51_CORREUS_PRINCIPALES
        ...
        --                      AND NOT EXISTS (SELECT * FROM Z_C55_TELEFONS_NUMERICS AUX  WHERE C.ID_CONTACTE_SINTAGMA = AUX.ID_CONTACTE_SINTAGMA AND REPLACE(REPLACE(REPLACE(TRIM(C.TEL_FIX),'-',''),' ',''),'.','') = AUX.TELEFON);
             ...
             INSERT INTO Z_C55_TELEFONS_NUMERICS (ID_CONTACTE_SINTAGMA, TIPUS_TELEFON, TELEFON)
              ...
              COMMIT;                      
--                      AND NOT EXISTS (SELECT * FROM Z_C55_TELEFONS_NUMERICS AUX  WHERE C.ID_CONTACTE_SINTAGMA = AUX.ID_CONTACTE_SINTAGMA AND REPLACE(REPLACE(REPLACE(TRIM(C.TEL_FIX_CENTRALETA),'-',''),' ',''),'.','') = AUX.TELEFON);
        ...
        --                      AND NOT EXISTS (SELECT * FROM Z_C55_TELEFONS_NUMERICS AUX  WHERE C.ID_CONTACTE_SINTAGMA = AUX.ID_CONTACTE_SINTAGMA AND REPLACE(REPLACE(REPLACE(TRIM(C.TEL_MOBIL),'-',''),' ',''),'.','') = AUX.TELEFON);
...
--                      AND NOT EXISTS (SELECT * FROM Z_C55_TELEFONS_NUMERICS AUX  WHERE C.ID_CONTACTE_SINTAGMA = AUX.ID_CONTACTE_SINTAGMA AND REPLACE(REPLACE(REPLACE(TRIM(C.FAX),'-',''),' ',''),'.','') = AUX.TELEFON);
      ...
      BEGIN
           ...
           INSERT INTO A0_CONTACTE_TELEFON (ID, NUMERO, ES_PRINCIPAL, DATA_CREACIO, USUARI_CREACIO, CONTACTE_ID, TIPUS_TELEFON_ID)
            ...
            WHERE EXISTS(SELECT * FROM A0_CONTACTE WHERE ID = NUEVOS.id_contacte) 
   ...
   /* (ANULADO) Obtengo las entidades únicas del históricoy su nombre normalizado asigignándole un ID de secuencia
   
   PROCEDURE C90_ENTITATS_TRUCADA IS
   BEGIN
        INSERT INTO  Z_C90_ENTITAT_TRUCADA
            SELECT  AUX_SEQ_ENTITAT_TRUCADA_CALIMA.NEXTVAL AS ENTITAT_ID, t2.ENTITAT_NOMALITZAT, t2.descripcio
            FROM (
                   SELECT t1.ENTITAT_NOMALITZAT AS ENTITAT_NOMALITZAT, truc1.ORGANISME_ AS descripcio
                    FROM( 		
                         SELECT LIMPIARcHARS(ORGANISME_) AS ENTITAT_NOMALITZAT		    
                         FROM Z_TMP_CALIMA_U_HIST_TRUCADA ENTITATS		    
                         GROUP BY LIMPIARcHARS(ORGANISME_)
                         ) t1,
     			           Z_TMP_CALIMA_U_HIST_TRUCADA truc1
		            WHERE t1.ENTITAT_NOMALITZAT = LimpiarChars(truc1.ORGANISME_)
		            GROUP BY t1.ENTITAT_NOMALITZAT,truc1.ORGANISME_
	             ) t2
        COMMIT;         
   END;
   */
   ...
   /*
        INSERT INTO  Z_C90_ENTITAT_TRUCADA
            SELECT  AUX_SEQ_ENTITAT_TRUCADA_CALIMA.NEXTVAL AS ENTITAT_ID,Group_Entitats.ENTITAT_NOMALITZAT AS ENTITAT_NOMALITZAT,Group_Entitats.Fecha_Creacio
            FROM ( 	
                    SELECT LIMPIARcHARS(ORGANISME_) AS ENTITAT_NOMALITZAT,min(TO_DATE(DARRERA_INTERV,'dd/mm/yyyy HH24:MI:SS')) AS Fecha_Creacio 		    
            ...
            LEFT OUTER JOIN 
                DM_ENTITAT
                ON   Group_Entitats.ENTITAT_NOMALITZAT = LimpiarChars(DM_ENTITAT.DESCRIPCIO) 
...
*/
            ...
            INSERT into Z_C90_ENTITAT_TRUCADA (ENTITAT_ID, ENTITAT, FECHA_CREACIO,ENTITAT_NORMALITZADA)                       
                ...
                LEFT OUTER JOIN 
                    A0_DM_ENTITAT
                    ON   FUNC_NORMALITZAR_ENTITAT(Group_Entitats.ENTITAT) = FUNC_NORMALITZAR_ENTITAT(A0_DM_ENTITAT.DESCRIPCIO)
                WHERE A0_DM_ENTITAT.DESCRIPCIO IS NULL
                  AND NOT EXISTS (SELECT 1 
  ...
  PROCEDURE C91_DM_ENTITATS_TRUCADAS IS
   BEGIN
          INSERT INTO A0_DM_ENTITAT (ID, CODI, DESCRIPCIO, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, ID_ORIGINAL, ESQUEMA_ORIGINAL, TABLA_ORIGINAL)
...
--               GROUP BY ID,DESCRIPCIO, DATA_CREACIO,DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO;
   ...
   BEGIN
        INSERT INTO A0_TRUCADA (ID, DATA_REGISTRE, DATA_RESOLUCIO, NOM, COGNOM1, COGNOM2, CARREC, TELEFON, TELEFON_SECUNDARI, DESCRIPCIO, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, ENTITAT_ID, ESTAT_TRUCADA_ID, DESTINATARI_PERSONA_ID, CONTACTE_ID, SENTIT_ID)
                    SELECT ID, DATA_REGISTRE, DATA_RESOLUCIO, NOM, COGNOM1, COGNOM2, CARREC, TELEFON, TELEFON_SECUNDARI, DESCRIPCIO, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, ENTITAT_ID, ESTAT_TRUCADA_ID, DESTINATARI_PERSONA_ID, CONTACTE_ID, SENTIT_ID
                    FROM (
                          SELECT ID,
                                 DATA_REGISTRE,
   ...
   BEGIN
     INSERT INTO A0_HISTORIC_TRUCADA (ID, DESCRIPCIO, SENTIT_ID, TELEFON, DATA_REGISTRE, DATA_CREACIO, DATA_ESBORRAT, DATA_MODIFICACIO, USUARI_CREACIO, USUARI_ESBORRAT, USUARI_MODIFICACIO, TRUCADA_ID, RAO_ID, ESTAT_TRUCADA_ID)
          SELECT N AS ID,
                 ASSUMPTE AS DESCRIPCIO,
                  (CASE WHEN E_S='E' THEN 
                        2
...
--        DELETE FROM Z_C20_CONTACTES_EXIST_SINTAGMA;
    ...
    BEGIN
        ...
        Tabla := V_Sequence('AUX_SEQ_CALIMA_CONTACTE_CORREU',
                            'AUX_SEQ_CALIMA_DM_DEST_PERSONA',
                            'AUX_SEQ_CALIMA_ENTITAT_TRUCADA',                            
                            'AUX_SEQ_CALIMA_DM_CARREC',
                            'AUX_SEQ_CALIMA_DM_TRACTAMENT',                             
                            'AUX_SEQ_CALIMA_ADRECA',
                            'AUX_SEQ_CALIMA_CONTACTE_TFN' ,
                            'AUX_SEQ_CALIMA_ENTITAT');
        ...
        FOR i in 1 .. total LOOP
                execute immediate 'DROP SEQUENCE ' || Tabla(i);           
                execute immediate 'CREATE SEQUENCE ' || Tabla(i) || ' MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 NOCACHE NOORDER NOCYCLE';            
...
/*
DROP SEQUENCE AUX_SEQ_CALIMA_CONTACTE_CORREU;
CREATE SEQUENCE AUX_SEQ_CALIMA_CONTACTE_CORREU MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 NOCACHE NOORDER NOCYCLE;

DROP SEQUENCE AUX_SEQ_CALIMA_DM_DEST_PERSONA;
CREATE SEQUENCE AUX_SEQ_CALIMA_DM_DEST_PERSONA MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 NOCACHE NOORDER NOCYCLE;

DROP SEQUENCE AUX_SEQ_CALIMA_ENTITAT_TRUCADA;
CREATE SEQUENCE AUX_SEQ_CALIMA_ENTITAT_TRUCADA MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 NOCACHE NOORDER NOCYCLE;

DROP SEQUENCE AUX_SEQ_CALIMA_DM_CARREC;
CREATE SEQUENCE AUX_SEQ_CALIMA_DM_CARREC MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 NOCACHE NOORDER NOCYCLE;

DROP SEQUENCE AUX_SEQ_CALIMA_DM_TRACTAMENT;
CREATE SEQUENCE AUX_SEQ_CALIMA_DM_TRACTAMENT MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 NOCACHE NOORDER NOCYCLE;


DROP SEQUENCE AUX_SEQ_CALIMA_ADRECA;
CREATE SEQUENCE AUX_SEQ_CALIMA_ADRECA MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 NOCACHE NOORDER NOCYCLE;

DROP SEQUENCE AUX_SEQ_CALIMA_CONTACTE_TFN;
CREATE SEQUENCE AUX_SEQ_CALIMA_CONTACTE_TFN MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 NOCACHE NOORDER NOCYCLE;

DROP SEQUENCE AUX_SEQ_CALIMA_ENTITAT;
CREATE SEQUENCE AUX_SEQ_CALIMA_ENTITAT MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 NOCACHE NOORDER NOCYCLE;

*/
    END;

END Z_LANZAR_AUX_CALIMA;
